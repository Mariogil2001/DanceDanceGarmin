...
class DanceDanceAnimationController {
    ...
    private function buildTextLayer() {
        ...
        var width = (info.screenWidth * .60).toNumber();
        var height = (info.screenHeight * .25).toNumber();
        
        var options = {
            :locX => ((info.screenWidth - width) / 2 ).toNumber(),
            :locY => ((info.screenHeight - height) / 2).toNumber(),
            :width => width,
            :height => height,
            :colorDepth => 1, // You may need to adjust this value
            :visibility => true
        };
